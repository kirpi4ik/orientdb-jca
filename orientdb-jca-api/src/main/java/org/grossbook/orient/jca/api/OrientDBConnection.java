...
public interface OrientDBConnection extends ODatabase {
	...
	/**
	 * Returns all the edges between the vertexes iVertex1 and iVertex2 with label between the array of labels passed as
	 * iLabels.
	 * 
	 * @param iVertex1
	 *            First Vertex
	...
	/**
	 * Returns all the edges between the vertexes iVertex1 and iVertex2 with label between the array of labels passed as
	 * iLabels and with class between the array of class names passed as iClassNames.
	 * 
	 * @param iVertex1
	 *            First Vertex
	 * @param iVertex2
	...
	/**
	 * Retrieves the outgoing edges of vertex iVertex having the requested properties iProperties set to the passed
	 * values
	 * 
	 * @param iVertex
	 *            Target vertex
	...
	/**
	 * Retrieves the incoming edges of vertex iVertex having the requested properties iProperties set to the passed
	 * values
	 * 
	 * @param iVertex
	 *            Target vertex
	...
	/**
	 * Saves a document to the database. Behavior depends by the current running transaction if any. If no transaction
	 * is running then changes apply immediately. If an Optimistic transaction is running then the record will be
	 * changed at commit time. The current transaction will continue to see the record as modified, while others not. If
	 * a Pessimistic transaction is running, then an exclusive lock is acquired against the record. Current transaction
	 ...
	 * {@link OConcurrentModificationException} exception is thrown.Before to save the document it must be valid
	 * following the constraints declared in the schema if any (can work also in schema-less mode). To validate the
	 * document the {@link ODocument#validate()} is called.
	 * 
	 * @param iRecord
	 *            Record to save.
	...
	/**
	 * Saves a document specifying a cluster where to store the record. Behavior depends by the current running
	 * transaction if any. If no transaction is running then changes apply immediately. If an Optimistic transaction is
	 * running then the record will be changed at commit time. The current transaction will continue to see the record
	 * as modified, while others not. If a Pessimistic transaction is running, then an exclusive lock is acquired
	 * against the record. Current transaction will continue to see the record as modified, while others cannot access
	 * to it since it's locked.
	 * <p/>
	 * If MVCC is enabled and the version of the document is different by the version stored in the database, then a
	 * {@link OConcurrentModificationException} exception is thrown. Before to save the document it must be valid
	 * following the constraints declared in the schema if any (can work also in schema-less mode). To validate the
	...
	/**
	 * Deletes a document. Behavior depends by the current running transaction if any. If no transaction is running then
	 * the record is deleted immediately. If an Optimistic transaction is running then the record will be deleted at
	 * commit time. The current transaction will continue to see the record as deleted, while others not. If a
	 * Pessimistic transaction is running, then an exclusive lock is acquired against the record. Current transaction
